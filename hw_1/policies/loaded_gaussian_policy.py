...
class LoadedGaussianPolicy(BasePolicy, nn.Module):
    ...
    def forward(self, obs):
        print(f'obs {obs}, obs_norm_mean {self.obs_norm_mean}, obs_norm_std {self.obs_norm_std}')
        normed_obs = (ptu.from_numpy(obs) - self.obs_norm_mean) / (self.obs_norm_std + 1e-6)
        h = normed_obs
